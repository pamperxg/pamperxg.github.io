...
>我们一般使用信息量来进行不确定性的度量。分布为p(x)随机变量X的自信息定义为：-log(p(x))。
>
>熵定义为信息量关于随机变量概率分布的期望（编码信息的比特位下界）。
>
>条件熵：条件概率分布的熵对随机变量X的期望，条件熵 = 联合熵 - 单独熵。
>
>相对熵（KL散度）：两个概率分布之间的差异，sum(pi*log(pi/qi))。
>
>交叉熵：模型的分布于训练时分布的差异，crossEntropy = H(label) - KL(label || q)，当KL散度为0的时候，交叉熵为0。
>
...
>最小二乘法等价于目标变量先验分布为高斯分布是的极大似然估计，明显逻辑回归目标变量的分布为伯努利分布。
