...
回归的线性模型和分类的线性模型：基于固定非线性基函数的线性组合。$y(x,w)=f(\sum_{j=1}^{M}w_j\phi_j(x))$,$f(.)$在分类问题中是一个非线性激活函数，在回归问题中为恒等函数。
...
固定基函数的线性组合构成的回归模型和分类模型，它们的实际应用被维数灾难问题限制了。为了将这些模型应用于大规模问题，有必要根据数据调节基函数。（SVM、神经网络）
